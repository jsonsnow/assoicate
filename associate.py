...
def cut_words(temp):
    ...
    seg_list = jieba.cut(wenben, cut_all=True)
    lists = []
...
def get_assoociate_word():
    word = input('请输入您想联想的词：')
    print("输入法词汇", word)
    print('请稍等')
    wenzhang = re.sub("[1234567890\s+\.\!\/_；：,$%^*(+\"\']+|[+——！，。？、~@#￥%……&*（）]+", "", temp)
    a = cut_words(wenzhang)
    b = get_countsl(a)
    # print(b)
    data = {}
...
if __name__ == '__main__':
    ...
    get_assoociate_word()  # 调用获取联想词的函数。